...
library(DAAG)
attach(Manitoba.lakes) 
plot(y = log(area), x = elevation, pch=16, xlim = c(170,260), log = "y")
# NB: 10 x area increases log(area) by 1.0
text(log(area) ~ elevation,
     labels=row.names(Manitoba.lakes), pos=4)
text(log(area) ~ elevation, labels=area, pos=2)
title("Manitoba’s Largest Lakes")
detach(Manitoba.lakes)
```
...
We want evaluate the probability from the joint p.d.f, in the region $\Omega = \{(x,y)|0\le x \le 0.5, 0\le y \le0.5\}$. 
We have that:
...
Let X and Y be non-independent random variables, such that $var(X) = σ^2x$, $var(Y ) = σ^2_y$ and $cov(X, Y ) = σ^2_{xy}$. Using the result from Section 1.2, find var(X + Y ) and var(X − Y ).
...
\end{aligned}
$$
Given the power series representation of the exponential function $\sum^\infty_{k=0}\frac{x^k}{k!} = e^x$, we can express $M_Y(s)$ as:
...
Consider independent Poisson variables $X_1,X_2,...,X_n$, $X_i \sim Pois(\lambda_i) \text{ for } i=1,...,n$. In the previous point we show that the sum of independent Poisson r.v. is a Poisson random variable $Y= \sum_{i=1}^nX_i \sim Pois(\lambda)$ , where $\lambda = \sum_{i=1}^{n} \lambda_i$
...
#PoissonGamma <- nb_mixture(df,n)
#plot( density(PoissonGamma), col="black", lwd=1, type="h",
#main="Negative Binomial distribution as a Poisson-Gamma mixture")
#plot(0:10000, dnbinom(0:10000, df, prob=p), xlab="Y=number of failures before k successes",
...
#egend("topright", legend=c("Poisson-Gamma Mixture", "Negative Binomial"), fill=c("black", "red"))
